...
)
{
    ...
    curandState* cur_state=&state[bni];
    ...
    for(int wi=wbeg;wi<w;wi+=interval)
    {
        ...
        if(dist<(dist_pos+margin))
        {
            float rand_val=curand_uniform(cur_state);
            cur_size+=1.f;
            if (rand_val<=1.f/cur_size)
            {
                curh=hi;
                curw=wi;
                curb=sbi;
...
)
{
    ...
    gpuErrchk(cudaMalloc(&devStates, b*n*sizeof(curandState)))
    setup_kernel<<<bdim,tdim>>>(devStates,unsigned(time(NULL)),b,n);
    gpuErrchk(cudaGetLastError())
