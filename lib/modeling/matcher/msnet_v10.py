...
class ScaleEstimator(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.linear = nn.Sequential(
            nn.Conv2d(3600, 256, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
        )
        ...
        #     nn.ReLU(True)
        # )
        self.scale_pred = nn.Conv2d(512, 3, kernel_size=1, stride=1, padding=0)
...
class MultiScaleNetV10(nn.Module):
    def __init__(self, cfg):
        super(MultiScaleNetV10, self).__init__()
    ...
    def inference(self, images0, images1):
        descs0 = self.desc_extractor(images0)
        descs1 = self.desc_extractor(images1)
