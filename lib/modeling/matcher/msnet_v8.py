...
class DescExtractor(nn.Module):
    ...
    def forward(self, images, scales, id):
        ...
        # for left image, if the scale is smaller than 1, left descriptor should adopt a downsample one
        # for right image, if the scale is bigger than 1, right descriptor should adopt a downsample one
        # if id == 'left':
        #     msk = (scale < 1).float()
...
class MultiScaleNetV8(nn.Module):
    def __init__(self, cfg):
        super(MultiScaleNetV8, self).__init__()
