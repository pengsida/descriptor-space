...
from lib.utils.base import convert_to_rgb, compute_homography_discrete, round_coordinates, random_crop, \
    draw_corspd, draw_kps, draw_corspd_region, compute_scale
from lib.data.datasets.coco import perspective, scale, translation, rotation


class COCODatasetV1(Dataset):
    def __init__(self, cfg, root, transforms=None):
        super(COCODatasetV1, self).__init__()
        # self.img_paths = get_img_path(root)[:1] * 100
        # self.img_paths = ['data/HPATCHES/v_graffiti/1.ppm']
...
def homography(h, w):
    ...
    patch_ratio = np.random.uniform(0.5, 0.8)
...
def homography_adaption(img0):
    h, w = img0.shape[:2]
    H = homography(h, w)
    scale_ratio = compute_scale(H, h, w)
