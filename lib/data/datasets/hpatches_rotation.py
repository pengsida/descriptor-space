...
from lib.utils.base import convert_to_rgb, compute_homography_discrete, round_coordinates, random_crop, \
    draw_corspd, draw_kps, draw_corspd_region, resize_max_length


class HpatchesRotation(Dataset):
...
def homography(h, w, **kargs):
    ...
    :return:
    """
    angle = kargs.get('angle', 0)
    patch_ratio = 1
    H_rotate =  cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1)
    H_rotate = np.vstack([H_rotate, np.array([0, 0, 1])])
    return H_rotate, patch_ratio


def homography_adaption(img0, **kargs):
    h, w = img0.shape[:2]
    H, patch_ratio = homography(h, w, **kargs)
    img1 = cv2.warpPerspective(img0, H, (w, h), flags=cv2.INTER_LINEAR)
    return img1, H


def get_img_path(root):
    img_paths = glob.glob(os.path.join(root, '*/*.ppm'))
    return img_paths
